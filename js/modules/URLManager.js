 ...
 */
export function updateURLParams(level, jurisdiction, status) {
    ...
    if (window.location.href !== new URL(newURL, window.location.origin).href) {
        window.history.pushState({ level, jurisdiction, status }, '', newURL);
    }
}
...
export function initializeURLState() {
    ...
    if (!window.history.state) {
        window.history.replaceState(
            { level: filters.level, jurisdiction: filters.jurisdiction, status: filters.status },
            '',
